...
va_list args;를 선언하여 가변 인자 목록을 관리합니다.
va_start(args, count);로 가변 인자 목록을 초기화합니다. 여기서 count는 인자의 개수입니다.
va_arg(args, int);를 사용하여 가변 인자의 각각의 요소를 순차적으로 접근합니다.
va_end(args);로 가변 인자 처리를 종료합니다.
소제목: 문자열 인자 처리
문자열 인자를 받는 함수
...
+ 배열 인자 함수 호출
++ int arr[] = { 1, 2, 3, 4, 5 };로 배열을 선언하고, printArray(arr, 5);와 printArrayptr(arr, 5);로 배열의 요소를 출력합니다.
