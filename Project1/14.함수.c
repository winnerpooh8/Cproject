...
//3. 매개변수는 함수가 팔요한 입력 값을 받기 위해 정의해주는 변수입니다, 외부로부터 전달받은 값을 함수 내부의 값으로써 사용하게 해주며
//   함수의 사용이 종료되면 매개변수도 삭제됩니다.
//4. return 코드를 실행하면, 함수가 종료됩니다. 추가적으로 return 다음에 값이 작성되어 있는 경우 함수를 호출한 위치에 그 값을 전달합니다.
...
int sub(int a, int b)
{
    return a - b; //간단한 계산 결과하면 return을 통해 한번에 작성하는게 위의 방식보다 읽기 편합니다.
}
